<template>
    ...
    <div class="flex justify-center items-center h-screen bg-gray-100 px-6">
        <div class="w-full max-w-xs">
            <h2 class="text-2xl mb-5 text-center">Login to your account</h2>
            <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
                <fieldset>
                    <BaseInput type="email" v-model="email.value.value" label="Email" placeholder="Enter your email" :error="errors.email" />
                    <BaseInput type="password" v-model="password.value.value" label="Password" placeholder="Enter your password" :error="errors.password" />
                </fieldset>
                <fieldset>
                    ...
                    <router-link :to="{ name: 'Register'}" class="text-blue-700">Create new account</router-link>
                </fieldset>
            </form>
        </div>
    </div>
...
const email = useField('email', Yup.string().required().email())
const password = useField('password', Yup.string().required())
...
const login = handleSubmit(async () => {
    ...
    console.log("email: ", email.value.value);
       ...
       try {
           let response = await apiService.store('users/login', credentials)
