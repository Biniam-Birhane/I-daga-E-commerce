<template>
    ...
    <div class="flex flex-col justify-center h-screen">
        <h3 class="mx-auto mb-6 font-semibold text-xl">Details about this product</h3>
        <div
            ...
            <div class="w-full md:w-2/3 bg-white flex flex-col space-y-2 p-3">
                ...
                <div class="flex justify-between items-center">
                    <p class="text-gray-500 font-medium hidden md:block">Owner: {{ owner.firstName + ' ' + owner.lastName }}
                    </p>
                    <p class="text-gray-500 font-medium hidden md:block">Category: {{ category.name }}</p>
                </div>
                <div class="pt-6">
                    <router-link :to=" {name: 'Products'}" class="font-semibold hover:underline text-blue-500">Go to products</router-link>
                </div>
            </div>
        </div>
    </div>
...
const getProduct = async () => {
    ...
    try {
        let response = await apiService.getOne(`products/${route.params.id}`)
