...
<template>
	<div class="container mx-auto my-1">
		<h1 class="text-2xl font-bold mb-4 ml-6">Products Table</h1>
						...
						<div class="flex flex-wrap -mx-3 mb-6">
							<div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
								...
								<input
									...
									:class="$v.productName.$error ? 'border-red-500' : 'border-gray-200'"
									v-model="editedItem.productName" id="grid-product-name" type="text"
									placeholder="Enter your product name">
								<p v-for="error in $v.productName.$errors" :key="error.$uid"
									class="text-red-500 text-xs italic">{{error.$message}}</p>
							</div>
							<div class="w-full md:w-1/2 px-3">
								...
								<input
									...
									:class="$v.brand.$error ? 'border-red-500' : 'border-gray-200'"
						...
						<div class="flex flex-wrap -mx-3 mb-6">
							<div class="w-full md:w-1/3 px-3">
								...
								<input
									...
									:class="$v.price.$error ? 'border-red-500' : 'border-gray-200'"
							...
							<div class="w-full md:w-1/3 px-3">
								...
								<input
									class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
									:class="$v.quantity.$error ? 'border-red-500' : 'border-gray-200'"
									v-model="editedItem.quantity" id="grid-quantity" type="number"
									placeholder="Enter product quantity">
								<p v-for="error in $v.quantity.$errors" :key="error.$uid"
									class="text-red-500 text-xs italic">{{error.$message}}</p>
							</div>
							<div class="w-full md:w-1/3 px-3">
								<label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2"
								...
								<input
									class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
									:class="$v.productionDate.$error ? 'border-red-500' : 'border-gray-200'"
									v-model="editedItem.productionDate" id="grid-date" type="date"
									placeholder="Enter product price">
								<p v-for="error in $v.productionDate.$errors" :key="error.$uid"
									class="text-red-500 text-xs italic">{{error.$message}}</p>
							</div>
						</div>
						<div class="flex flex-wrap -mx-3 mb-2">
							<div class="w-full md:w-1/3 px-3 mb-6 md:mb-0">
								...
								<div class="relative">
									...
									<p v-for="error in $v.categoryId.$errors" :key="error.$uid" class="text-red-500 text-xs italic">
										{{error.$message}}</p>
								</div>
							</div>
							<div class="w-full md:w-1/3 px-3 mb-6 md:mb-0">
							...
							<div class="w-full md:w-1/3 px-3 mb-6 md:mb-0">
								...
								<input
									...
									:class="$v.image.$error ? 'border-red-500' : 'border-gray-200'"
...
const goTodetail = (itemId) => {
	console.log("item-id from product: ", itemId);
...
const saveProduct = async () => {
	...
	if (validationResult) {
		...
		try {
			...
			// })
			// formData.append('editedItem', editedItem.value)
...
const getAllProducts = async () => {
	...
	try {
		if (loggedInUser.role == 'Admin') {
			let response = await apiService.all('products')
